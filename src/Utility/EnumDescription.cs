...
namespace Daan.DataTransfer.Utility
{
    ...
    /// 
    /// 获得枚举类型的文字描述
    /// string txt = EnumDescription.GetEnumText(typeof(MyEnum));
    /// 获得某个枚举值的文字描述
    /// string txt = EnumDescription.GetFieldText(MyEnum.Two);
    /// 获得枚举中各个字段的定义数组：
    /// EnumDescription[] des = EnumDescription.GetFieldTexts(typeof(MyEnum)); 
    /// 如果需要排序，通过他的另一个重载  
